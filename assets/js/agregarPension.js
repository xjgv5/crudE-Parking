...
document.addEventListener('DOMContentLoaded', () => {
    ...
    get(pensionesRef)
      .then((snapshot) => {
        if (snapshot.exists()) {
          ...
          // Recorre todos los registros y recalcula los días por vencer
          for (const id in pensiones) {
            const pension = pensiones[id];
            const { fechaVencimiento, diasPorVencer, estado } = calcularFechaVencimientoYdias(
              pension.fechaContratacion,
              pension.mesesPagados
            );

            // Actualiza los campos en el registro
            pension.fechaVencimiento = fechaVencimiento;
            pension.diasPorVencer = diasPorVencer;
            pension.estado = estado;

            resultados.push(pension);
          }

          // Muestra los resultados en la tabla
          mostrarPensionesEnTabla(resultados);
        } else {
          console.log("No se encontraron registros de pensiones.");
        }
      })
  ...
  // Resto del código (abrirModalEditar, actualizarPension, eliminarPension)...
    // Función para abrir el modal de edición
    function abrirModalEditar(id, pension) {
      // Llena el formulario con los datos del registro
      document.getElementById('numeroTag').value = pension.numeroTag || "";
